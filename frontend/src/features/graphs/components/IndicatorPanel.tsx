...
const IndicatorPanel: React.FC<IndicatorPanelProps> = memo(({
  ...
  useEffect(() => {
    ...
    const initChart = () => {
      ...
      const chart = createChart(containerEl, getIndicatorChartOptions(isDarkMode));
  ...
  const updateLegendContent = useCallback(
    (param: MouseEventParams<Time>) => {
      ...
      const parts: string[] = [`<span class="font-semibold">${displayLabel}</span>`];
      ...
      seriesRefs.current.forEach((series, key) => {
        ...
        if (data && 'value' in data) {
          ...
          const color = (instance.config[`${key}Color`] as string) || outputDef?.defaultColor || '#888';
          parts.push(
      ...
      const html = parts.join(' <span class="text-muted-foreground">|</span> ');
  ...
  useEffect(() => {
    if (!chartReady || !chartRef.current || !output || !isInitializedRef.current) return;

    const chart = chartRef.current;
    const outputType = output.type;
    
    // Only clear series if output type changed
    if (prevOutputTypeRef.current !== null && prevOutputTypeRef.current !== outputType) {
    ...
    const canDoIncrementalUpdate = (data: { time: Time; value: number }[]): boolean => {
      if (data.length === 0) return false;
      const lastPoint = data[data.length - 1];
      return prevDataLengthRef.current > 0 && 
    ...
    if (output.type === 'line') {
      ...
      let series = seriesRefs.current.get(seriesKey) as ISeriesApi<'Line'> | undefined;
      const color =
        (instance.config.color as string) || outputDef?.defaultColor || '#F59E0B';
      ...
      if (lineOutput.data.length > 0) {
        lastTimeRef.current = lineOutput.data[lineOutput.data.length - 1].time;
      ...
      Object.entries(multiOutput.series).forEach(([key, data]) => {
        ...
        if (outputDef.type === 'histogram') {
          const color =
            (instance.config[`${key}Color`] as string) || outputDef.defaultColor;
          ...
          if (histData.length > 0) {
            ...
            const isSameLength = prevDataLengthRef.current > 0 && histData.length === prevDataLengthRef.current;
    ...
    if (legendRef.current) {
      ...
      seriesRefs.current.forEach((_series, key) => {
        ...
        const label = outputDef?.label || key;
        const color =
          (instance.config[`${key}Color`] as string) || outputDef?.defaultColor || '#888';
    ...
    if (definition.referenceLines && definition.referenceLines.length > 0) {
      definition.referenceLines.forEach((refLine, idx) => {
        ...
        if (output.type === 'line' && (output as LineIndicatorOutput).data.length > 0) {
          const lineOutput = output as LineIndicatorOutput;
          const refData: LineData<Time>[] = [
            { time: lineOutput.data[0].time, value: refLine.value },
            { time: lineOutput.data[lineOutput.data.length - 1].time, value: refLine.value },
          ];

          const refSeries = chart.addSeries(LineSeries, {
            color: refLine.color || '#6B7280',
            lineWidth: 1,
            lineStyle: refLine.style === 'dashed' ? 2 : refLine.style === 'dotted' ? 1 : 0,
  ...
  }, [chartReady, output, definition, instance.config, displayLabel, mainTimeScale]);
