...
class AssetViewSet(PublicReadOnlyMixin, viewsets.ReadOnlyModelViewSet):
    ...
    @action(detail=True, methods=["get"], url_path="option_chain")
    def option_chain(self, request, pk=None):
        """
        Fetch the options chain (snapshots with quotes and greeks) for an underlying equity asset.

        Query Parameters:
            expiration_date_gte: Filter options expiring on or after this date (YYYY-MM-DD).
            expiration_date_lte: Filter options expiring on or before this date (YYYY-MM-DD).
            type: Option type filter – 'call' or 'put'.
            limit: Maximum number of contracts to return (default 200, max 1000).
        ...
        Returns:
            {
                "msg": "Ok",
                "data": {
                    "snapshots": { "<symbol>": { "details": {...}, "greeks": {...}, ... } },
        ...
        if asset.asset_class != "us_equity":
            return Response(
                {"msg": "Options chain is only available for equity assets"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        expiration_date_gte = request.query_params.get("expiration_date_gte")
        expiration_date_lte = request.query_params.get("expiration_date_lte")
        option_type = request.query_params.get("type")
        limit = self._parse_limit(request, default=200, maximum=1000)
        page_token = request.query_params.get("page_token")

        try:
            data = alpaca_service.get_option_snapshots(
            ...
            return Response({"msg": "Ok", "data": data}, status=status.HTTP_200_OK)
        except Exception as e:
            logger.error(
    ...
    @action(detail=False, methods=["get"], url_path="option_bars")
    def option_bars(self, request):
        """
        Fetch historical OHLCV bars for a specific option contract.

        Query Parameters:
            symbol: Option contract symbol (required), e.g. AAPL240119C00180000.
            timeframe: Bar timeframe – '1Min', '5Min', '1Hour', '1Day' (default '1Day').
        ...
        limit = self._parse_limit(request, default=1000, maximum=10000)

        try:
            data = alpaca_service.get_option_bars(
                symbols=symbol,
                timeframe=timeframe,
                start=start,
                end=end,
